      ...
      >
        <el-table-column label="商品图片" width="160" align="center">
          <template slot-scope="scope">
            <img style="height: 80px" :src="scope.row.clothingPic" />
          </template>
        </el-table-column>
        <el-table-column label="商品名称" align="center">
          <template slot-scope="scope">
            <span class="font-small">{{ scope.row.clothingName }}</span
            ><br />
            <span class="font-small">品牌：{{ scope.row.clothingBrand }}</span>
          </template>
        </el-table-column>
        <el-table-column label="价格/货号" width="180" align="center">
          <template slot-scope="scope">
            <span class="font-small"
              >价格：￥{{ scope.row.clothingRealPrice }}</span
            ><br />
            <span class="font-small">货号：NO.{{ scope.row.clothingId }}</span>
          </template>
        </el-table-column>
        <el-table-column label="属性" width="180" align="center">
          <template slot-scope="scope">{{
            scope.row.clothingAttr
          }}</template>
        </el-table-column>
  ...
  computed: {
    totalAmount() {
      if (this.orderReturnApply != null) {
        return (
          this.orderReturnApply.clothingRealPrice *
          this.orderReturnApply.clothingCount
        );
      } else {
        return 0;
      }
  ...
  methods: {
    ...
    getDetail() {
      getApplyDetail(this.id).then((response) => {
        ...
        this.goodsList = [];
        this.goodsList.push(this.orderReturnApply);
