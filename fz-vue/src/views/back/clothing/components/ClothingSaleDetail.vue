  ...
  <div style="margin-top: 50px">
    <el-form :model="value" ref="clothingSaleForm" label-width="120px" style="width: 600px" size="small">
      ...
      <el-form-item v-show="value.promotionType===3">
        <el-table :data="value.clothingLadderList"
          ...
          <el-table-column
            ...
            label="操作">
            <template slot-scope="scope">
              <el-button type="text" @click="handleRemoveClothingLadder(scope.$index, scope.row)">删除</el-button>
              <el-button type="text" @click="handleAddClothingLadder(scope.$index, scope.row)">添加</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-form-item>
      <el-form-item v-show="value.promotionType===4">
        <el-table :data="value.clothingFullReductionList"
                  style="width: 80%" border>
          <el-table-column
            label="满"
            align="center"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleRemoveClothingLadder(index, row) {
        let clothingLadderList = this.value.clothingLadderList;
        if (clothingLadderList.length === 1) {
          clothingLadderList.pop();
          clothingLadderList.push({
            count: 0,
            discount: 0,
            price: 0
          })
        } else {
          clothingLadderList.splice(index, 1);
        }
      },
      handleAddClothingLadder(index, row) {
        let clothingLadderList = this.value.clothingLadderList;
        if (clothingLadderList.length < 3) {
          clothingLadderList.push({
            count: 0,
            discount: 0,
            price: 0
      ...
      handleRemoveFullReduction(index, row) {
        let fullReductionList = this.value.clothingFullReductionList;
